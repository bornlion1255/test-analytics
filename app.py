...
def load_gsheet_data():
    try:
        ...
        # Очистка основных колонок
        for col in ['Отдел', 'Статус', 'Тип обращения']:
            if col in df.columns: 
                df[col] = df[col].astype(str).str.strip().replace(['nan', ''], '-')
        
        # Твоя логика прямой маршрутизации
        def fix_topic(row):
...
# ==========================================
# TAB 3: DEPT ANALYSIS (FULL & CORRECTED)
# ==========================================
with tabs[2]:
    st.subheader("Детальный анализ по отделу")
    if not df_api.empty:
        all_depts = sorted(df_api['Отдел'].unique())
        selected_dept = st.selectbox("Выберите отдел", all_depts, key="dept_analysis_final")
        
        if selected_dept:
            # 1. Фильтруем данные API для KPI
            ...
            **(По отделу: Ср. скорость: {format_seconds(d_med)} | Рейтинг: {d_rate_str} ({d_rate_cnt} шт.))**
            ...
            # --- 4. ТЕМАТИКИ С УЧЕТОМ СТАТУСОВ (УНИВЕРСАЛЬНАЯ ЛОГИКА) ---
            st.subheader("Тематика обращений (GSheet)")
            dept_gsheet = df_gsheet[df_gsheet['Отдел'] == selected_dept].copy()
            
            # ФУНКЦИЯ ДЛЯ ФОРМИРОВАНИЯ КАТЕГОРИИ
            def get_universal_label(row):
                st_val = str(row.get('Статус', '-')).strip()
                tp_val = str(row.get('Тип обращения', '-')).strip()
                
                # Стандартные статусы, которые мы игнорируем (просто берем Тип)
                # Все остальное (Курьеры, Ручники, Ошибки, Рассылки) - выводим!
            ...
            if not dept_gsheet.empty:
                # 1. Применяем логику ко ВСЕМ строкам таблицы по этому отделу
                ...
                # 2. СЧИТАЕМ ИТОГО ПО ТАБЛИЦЕ
                # Теперь сюда входят и курьеры, и ручники, и рассылки
                total_sheet_found = len(dept_gsheet)
                
                # 3. СЧИТАЕМ ЧЕСТНУЮ РАЗНИЦУ
                # (Всего чатов в API - Все что нашли в Таблице)
                ...
                # Сортируем: сначала самые частые, разница обычно в топе или внизу
                ...
                st.warning(f"В GSheet нет данных для {selected_dept}. Разница с API: {d_chats_api}")
