...
# ==========================================
# ОБНОВЛЕННЫЙ БЛОК TAB 1: KPI
# ==========================================
with tabs[0]:
    st.subheader("📊 Единое распределение нагрузки")

    # --- 1. Подготовка данных для единого круга ---
    
    # А. Чаты людей по отделам (из API)
    if not df_api.empty:
        # Группируем уникальные чаты по отделам
        human_dept_data = df_api.groupby('Отдел')['req_id'].nunique().to_dict()
    else:
        human_dept_data = {}

    # Б. Системные и ботовые категории (из GSheet)
    # 1. Заглушка на старый чат (Без участия человека)
    stub_mask = df_gsheet['Тип обращения'].str.contains('Заглушка на старый чат', case=False, na=False)
    count_stubs = len(df_gsheet[stub_mask])

    # 2. Авторизация (делим на пройденную и нет)
    ...
    # 3. Бот закрыл чат (Бот принимал участие и сам завершил)
    # Используем count_bot_closed, который уже рассчитан выше в коде
    
    # В. Обработка "Без AI" (Человек обрабатывал по категориям)
    # Если в API чат не попал (например, не было исходящего), но в GSheet стоит "Без AI", 
    # мы берем категорию из Типа обращения.
    no_ai_mask = df_gsheet['Статус'].str.contains('Без AI', case=False, na=False)
    df_no_ai = df_gsheet[no_ai_mask]
    no_ai_counts = df_no_ai['Тип обращения'].value_counts().to_dict()

    # --- 2. Сборка финального словаря для графика ---
    # Собираем всё в один список категорий и значений
    labels = []
    values = []
    colors = []

    # Группа: Человек (Отделы из API)
    for dept, count in human_dept_data.items():
        labels.append(f"👨‍💻 {dept}")
        values.append(count)
        colors.append('#66b3ff') # Голубой для людей

    # Группа: Человек (Специфические случаи из GSheet)
    if count_auth_failed > 0:
        labels.append("⚠️ Ошибка авторизации (Человек)")
        values.append(count_auth_failed)
        colors.append('#3399ff')

    for topic, count in no_ai_counts.items():
        labels.append(f"📝 {topic} (Без AI)")
        values.append(count)
        colors.append('#0066cc')

    # Группа: Бот (Схожие цвета)
    if count_bot_closed > 0:
        labels.append("🤖 Бот: Закрыл сам")
        values.append(count_bot_closed)
        colors.append('#ff9999') # Розовый

    if count_auth_passed > 0:
        labels.append("🤖 Бот: Авторизация")
        values.append(count_auth_passed)
        colors.append('#ffb3b3') # Светло-розовый

    # Группа: Система
    if count_stubs > 0:
        labels.append("⚙️ Система: Заглушка")
        values.append(count_stubs)
        colors.append('#d3d3d3') # Серый
    ...
    if sum(values) > 0:
        fig_total = px.pie(
            ...
            title="Распределение всех входящих обращений"
        )
        fig_total.update_traces(textposition='inside', textinfo='percent+label')
        fig_total.update_layout(showlegend=True)
    ...
    # Оставляем метрики для быстрого взгляда
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Всего обращений", sum(values))
    c2.metric("Участие людей", sum([v for l, v in zip(labels, values) if "👨‍💻" in l or "⚠️" in l or "📝" in l]))
    c3.metric("Работа бота", count_bot_closed + count_auth_passed)
    c4.metric("Системные", count_stubs)
