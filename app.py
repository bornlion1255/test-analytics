...
def load_gsheet_data():
    try:
        ...
        # Чистим основные колонки от лишних пробелов и пустот
        for col in ['Отдел', 'Статус', 'Тип обращения']:
            if col in df.columns: 
                df[col] = df[col].astype(str).str.strip().replace(['nan', ''], '-')
        
        # Логика прямой маршрутизации (оставляем без изменений)
        def fix_topic(row):
    ...
    if not df_api.empty:
        ...
        if selected_dept:
            ...
            **(По отделу: Ср. скорость: {format_seconds(d_med)} | Рейтинг: {d_rate_str} ({d_rate_cnt} шт.))**
            ...
            # --- 5. ТЕМАТИКИ С РАСЧЕТОМ РАЗНИЦЫ (УЛУЧШЕННАЯ ЛОГИКА) ---
            st.subheader("Тематика обращений (GSheet)")
            dept_gsheet = df_gsheet[df_gsheet['Отдел'] == selected_dept].copy()
            
            # Функция для определения "Умной категории"
            def get_smart_cat(row):
                st_val = str(row.get('Статус', '')).strip()
                tp_val = str(row.get('Тип обращения', '')).strip()

                # Приоритет 1: Специфические статусы
                if st_val == 'Меню курьеров': return '🚚 Меню курьеров'
                if st_val in ['Ручник: Позовите человека', 'Ручник: Обзвон и отмены']: return '✅ Подтверждение заказов (Ручник)'
                
                # Приоритет 2: Ключевые слова в Статусе или Типе
                if 'Ответ на рассылку' in st_val or 'Ответ на рассылку' in tp_val: return '📩 Ответ на рассылку'
                if 'Ответ на макрос' in st_val or 'Ответ на макрос' in tp_val: return '⌨️ Ответ на макрос'

                # Приоритет 3: Если ничего не подошло, берем очищенный Тип обращения
                return tp_val

            # Применяем логику и считаем
            dept_gsheet['Категория_финальная'] = dept_gsheet.apply(get_smart_cat, axis=1)
            cat_counts = dept_gsheet['Категория_финальная'].value_counts().reset_index()
            cat_counts.columns = ['Категория', 'Кол-во']
            
            # Считаем разницу только ПОСЛЕ того, как учли все строки из GSheet
