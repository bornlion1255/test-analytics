...
with tabs[3]:
    ...
    # ДОБАВЛЕНО: Третья вкладка "Продукты"
    sub_tab1, sub_tab2, sub_tab3 = st.tabs(["📋 Полная детализация", "📈 Интерактивный ТОП-15", "📦 Продукты"])
    ...
    if not df_gsheet.empty:
        ...
        # Функция для вытаскивания типа юзера из скобок в конце строки
        ...
        # --- 2. SUB-TAB 1: ТАБЛИЦА С ДЕТАЛИЗАЦИЕЙ (ОБНОВЛЕННАЯ) ---
        with sub_tab1:
            st.write("#### Полная статистика по всем категориям")
            
            # Теперь группируем и по Продукту тоже
            stats = df_analysis.groupby(['Продукт', 'Тип обращения', 'Статус']).size().unstack(fill_value=0)
            ...
            def get_cat_details(row):
                ...
                # row.name[0] - Продукт, row.name[1] - Тип обращения
                cat_reasons = df_analysis[(df_analysis['Продукт'] == row.name[0]) & 
                                          (df_analysis['Тип обращения'] == row.name[1]) & 
                                          (df_analysis['Статус'] == 'Перевод')]
        ...
        with sub_tab2:
            ...
            color_map = {
                'Бот справился': '#26A69A',
                'Перевод: Не знает ответ': '#FF5252',
                'Перевод: Требует сценарий': '#FFAB40',
                'Перевод: Лимит сообщений': '#7C4DFF',
                'Перевод: Прочее': '#90A4AE',
                'Без статуса': '#CFD8DC'
            }
        ...
        # --- 4. SUB-TAB 3: ПРОДУКТЫ И ТИПЫ ЮЗЕРОВ (НОВЫЙ БЛОК) ---
        with sub_tab3:
            st.write("#### Анализ эффективности по Продуктам и Аудитории")
            
            # Группируем по новой связке: Продукт + Тип юзера
            prod_stats = df_analysis.groupby(['Продукт', 'Тип юзера', 'Статус']).size().unstack(fill_value=0)
            ...
            def get_prod_details(row):
                transferred = row.get('Перевод', 0)
                if transferred == 0: return "—"
                # Ищем переводы по Продукту и Типу юзера
                cat_reasons = df_analysis[(df_analysis['Продукт'] == row.name[0]) & 
                                          (df_analysis['Тип юзера'] == row.name[1]) & 
                                          (df_analysis['Статус'] == 'Перевод')]
                r_counts = cat_reasons['Причина перевода'].value_counts()
                return "\n".join([f"• {r}: {(count/transferred*100):.0f}%" for r, count in r_counts.items() if count > 0])
            ...
            # Сортируем сначала по продукту, потом по объему
