    ...
    if not df_api.empty:
        ...
        if selected_dept:
            ...
            def calc_metrics_for_report(df, sm, fsm, dept_name):
                ...
                ratings = pd.to_numeric(df.drop_duplicates('req_id')['rating'], errors='coerce').dropna() if not df.empty else pd.Series()
                ...
                daily_c = df.groupby('Дата')['req_id'].nunique() if not df.empty else pd.Series()
                if dept_name == "Бот AI":
                    daily_o = pd.Series(1, index=daily_c.index) if not daily_c.empty else pd.Series()
                else:
                    daily_o = df[~df['is_tl']].groupby('Дата')['operator_id'].nunique() if not df.empty else pd.Series()
            ...
            # --- ИЗОЛИРОВАННАЯ ЛОГИКА ДЛЯ БОТА (ПОДМЕНА ОБЪЕМОВ ИЗ GSHEET) ---
            if selected_dept == "Бот AI":
                # Отфильтровываем все участия бота из таблицы (Закрыл + Перевел)
                dept_gsheet = df_gsheet[df_gsheet['Статус'].isin(['Закрыл', 'Перевод'])].copy()
                d_chats_api = len(dept_gsheet) # Теперь Главная цифра = таблице (например, 3863)
                ...
                # Подменяем объемы в текущем отчете
                if curr_m:
                    ...
                    daily_c = dept_gsheet.groupby(dept_gsheet['Дата'].dt.date).size()
                    curr_m = {'chats': len(dept_gsheet), 'ratings': 0, 'csat': 0, 'specs': 1, 'load': round(daily_c.mean()) if not daily_c.empty else 0, 'speed': 0, 'first_speed': 0}
                    
                # Подменяем объемы в прошлом отчете
                if prev_m:
                    ...
                    daily_c_p = dept_gsheet_prev.groupby(dept_gsheet_prev['Дата'].dt.date).size()
                    prev_m = {'chats': len(dept_gsheet_prev), 'ratings': 0, 'csat': 0, 'specs': 1, 'load': round(daily_c_p.mean()) if not daily_c_p.empty else 0, 'speed': 0, 'first_speed': 0}
            else:
            ...
            if curr_m:
                ...
                report_text = f"""**{sel_start.strftime('%d.%m')} - {sel_end.strftime('%d.%m')}** **{selected_dept}** Всего чатов: {curr_m['chats']}{fmt_trend(curr_m['chats'], prev_m['chats'] if prev_m else 0)}  
...
Средняя скорость: {format_seconds(curr_m['speed'])}{fmt_trend(curr_m['speed'], prev_m['speed'] if prev_m else 0, is_time=True)}"""
            ...
            if selected_dept == "Бот AI":
                ...
                st.caption("ℹ️ **Важно:** Скорость и оценки бота берутся из API (там, где бот успел отправить сообщение). Объем чатов и конверсия берутся из Google Таблицы (все системные закрытия).")
            ...
            if selected_dept == "Бот AI":
                ...
                daily_chats, daily_ops = pd.Series(), pd.Series()
