...
with tabs[0]:
    ...
    cols[3].metric("Бот (Закрыл)", count_bot_closed, f"{count_bot_closed/max(1, participated_count)*100:.1f}%")
    ...
    with col_pies[0]:
        ...
        if total_chats_all > 0:
            ...
            # Дополнительная инфо-панель под первым кругом
            st.write(f"🤖 **Общий охват бота:** {participated_count} диалогов")
            st.write(f"⚙️ **Технические (Заглушки):** {count_stub} шт.")

    with col_pies[1]:
        # Сдвигаем кружок эффективности вниз с помощью пустых строк или отступа
        st.write("##") 
        st.write("##")
        ...
        if participated_count > 0:
            ...
            # Тоже делаем Donut для стиля
            centre_circle2 = plt.Circle((0,0), 0.70, fc='white')
            fig2.gca().add_artist(centre_circle2)
            
            st.pyplot(fig2, use_container_width=False)
            
            # Детализация ручной работы (то, что просил добавить)
            st.markdown(f"""
            **Ручное участие в спец. задачах:**
