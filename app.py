    ...
    if not df_api.empty:
        ...
        if selected_dept:
            ...
            **(По отделу: Ср. скорость: {format_seconds(d_med)} | Рейтинг: {d_rate_str} ({d_rate_cnt} шт.))**
            ...
            # (Твой блок со списком специалистов без изменений)
            ...
            # --- 5. ТЕМАТИКИ С РАСЧЕТОМ РАЗНИЦЫ (ИСПРАВЛЕННАЯ ЛОГИКА) ---
            st.subheader("Тематика обращений (GSheet)")
            dept_gsheet = df_gsheet[df_gsheet['Отдел'] == selected_dept].copy()
            
            # ФУНКЦИЯ ДЛЯ ФИНАЛЬНОГО ИМЕНИ (Сначала проверяем Статус)
            def get_final_label(row):
                st_val = str(row.get('Статус', '-')).strip()
                tp_val = str(row.get('Тип обращения', '-')).strip()
                
                if st_val == 'Меню курьеров':
                    return f"Меню курьера ({tp_val})"
                if st_val in ['Ручник: Позовите человека', 'Ручник: Обзвон и отмены']:
                    return f"Подтверждение заказов ({tp_val})"
                if 'Ответ на рассылку' in st_val or 'Ответ на рассылку' in tp_val:
                    return f"Ответ на рассылку ({tp_val})"
                if 'Ответ на макрос' in st_val or 'Ответ на макрос' in tp_val:
                    return f"Ответ на макрос ({tp_val})"
                
                return tp_val

            # Применяем умную логику
            dept_gsheet['Категория_Финальная'] = dept_gsheet.apply(get_final_label, axis=1)
            
            # Считаем чаты по умным категориям
            cat_counts = dept_gsheet['Категория_Финальная'].value_counts().reset_index()
            cat_counts.columns = ['Категория', 'Кол-во']
            
            # Общее кол-во найденных в таблице чатов (теперь они все задействованы!)
            total_sheet = len(dept_gsheet)
            
            # Разница считается только теперь
