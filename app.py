...
with tabs[0]:
    ...
    # Группа "ЧЕЛОВЕК" (Синие/Голубые тона)
    human_segments = {}
    
    # А. Чаты отделов из API
    if not df_api.empty:
        human_segments.update(df_api.groupby('Отдел')['req_id'].nunique().to_dict())

    # Б. Категории "Без AI" (Человек обрабатывал вручную)
    ...
    for cat, count in no_ai_counts.items():
        human_segments[f"📝 {cat} (Без AI)"] = count

    # В. Ошибка авторизации (Требует человека)
    ...
    if count_auth_failed > 0:
        human_segments["⚠️ Ошибка авторизации (Человек)"] = count_auth_failed

    # Группа "БОТ / СИСТЕМА" (Красные/Оранжевые/Серые тона)
    bot_system_segments = {}

    # А. Бот: Закрыл сам (Участие бота 100%)
    count_bot_closed = len(df_gsheet[df_gsheet['Статус'].str.lower() == 'закрыл'])
    if count_bot_closed > 0:
        bot_system_segments["🤖 Бот: Закрыл сам"] = count_bot_closed

    # Б. Бот: Успешная авторизация
    ...
    if count_auth_passed > 0:
        bot_system_segments["🤖 Бот: Авторизация"] = count_auth_passed

    # В. Система: Заглушки (Объединяем все старые и новые в одну категорию)
    stub_mask = df_gsheet['Тип обращения'].str.contains('Заглушка', case=False, na=False)
    count_stubs = len(df_gsheet[stub_mask])
    if count_stubs > 0:
        bot_system_segments["⚙️ Система: Заглушка"] = count_stubs

    # --- 2. СБОРКА ДЛЯ ГРАФИКА ---
    labels = list(human_segments.keys()) + list(bot_system_segments.keys())
    values = list(human_segments.values()) + list(bot_system_segments.values())
    
    # Цветовая палитра: Человек (Синие), Бот (Красные/Оранжевые), Система (Серый)
    colors = (
        ['#1f77b4', '#3498db', '#5dade2', '#85c1e9', '#aed6f1'] * 5 # Синие оттенки для людей
    )[:len(human_segments)] + (
        ['#e74c3c', '#e67e22', '#f1948a'] # Красные/Оранжевые для бота
    )[:len(bot_system_segments) - 1] + ['#bdc3c7'] # Серый для системы

    # --- 3. ОТРИСОВКА (Plotly Pie) ---
    if sum(values) > 0:
        fig = go.Figure(data=[go.Pie(
            labels=labels, 
            values=values, 
            hole=0, # Сплошной круг, без дырки в центре
            marker_colors=colors,
            textinfo='none', # Убираем текст с сегментов
            hoverinfo='label+percent+value',
            sort=False
        )])

        fig.update_layout(
            height=700, # Делаем график крупным
            ...
            title=dict(
                text="<b>Распределение всех чатов: Участие бота vs Человек</b>",
                x=0.5,
                font=dict(size=20)
            )
        )
    ...
    human_vol = sum(human_segments.values())
    bot_vol = count_bot_closed + count_auth_passed
    
    c1.metric("Всего обращений", total_vol)
    c2.metric("Участие людей", human_vol)
    c3.metric("Закрыто ботом", bot_vol, f"{round(bot_vol/total_vol*100, 1)}% от всех")
