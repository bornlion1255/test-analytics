...
with tabs[0]:
    ...
    # Заглушки (новый пункт)
    stub_mask = (df_gsheet['Тип обращения'] == 'Заглушка на старый чат')
    count_stub = len(df_gsheet[stub_mask])
    
    # Люди
    pure_human_chats = max(0, count_human_chats - transferred_count)
    
    # Общий итог (обновляем с учетом заглушек)
    total_chats_all = count_human_chats + count_bot_closed + count_auth + count_stub
    
    # --- 2. KPI ПАНЕЛЬ ---
    cols = st.columns(6) # Увеличили до 6 колонок
    cols[0].metric("Всего чатов", total_chats_all)
    cols[1].metric("Участие бота", participated_count)
    cols[2].metric("Бот (Закрыл)", count_bot_closed, f"{count_bot_closed/max(1, participated_count)*100:.1f}%")
    cols[3].metric("Люди (Всего)", count_human_chats)
    cols[4].metric("Заглушка", count_stub) # Новая метрика
    cols[5].metric("Авторизация", count_auth)
    ...
    with col_pies[0]:
        ...
        if total_chats_all > 0:
            ...
            sizes = [count_bot_closed, transferred_count, pure_human_chats, count_stub, count_auth]
            
            # Цвета: добавили серый для заглушек
            ...
            wedges, texts, autotexts = ax1.pie(
                ...
                startangle=90,
                pctdistance=0.85,
                explode=[0.05 if i == 0 else 0 for i in range(len(labels))] # Слегка выдвигаем "Бот Закрыл"
            )
            ...
            # Информационные плашки под графиком
            st.write(f"🤖 **Общий охват бота:** {participated_count} диалогов")
            st.write(f"⚙️ **Технические (Заглушки):** {count_stub} шт.")
    ...
    with col_pies[1]:
        ...
        if participated_count > 0:
            ...
            st.write("Бот не участвовал в диалогах.")
