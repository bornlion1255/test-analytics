...
with tabs[0]:
    ...
    # --- 1. РАСЧЕТ ДОПОЛНИТЕЛЬНЫХ МЕТРИК ---
    bot_participated_df = df_gsheet[df_gsheet['Статус'].isin(['Закрыл', 'Перевод'])]
    participated_count = len(bot_participated_df) # Где бот принимал участие
    transferred_count = participated_count - count_bot_closed # Перевел на людей
    
    # "Чистые люди" — это те, кто в API есть, но не являются результатом перевода ботом (упрощенно)
    pure_human_chats = max(0, count_human_chats - transferred_count)
    
    # --- 2. KPI ПАНЕЛЬ ---
    c1, c2, c3, c4, c5 = st.columns(5)
    c1.metric("Всего чатов", total_chats_day)
    c2.metric("Участие бота", participated_count, help="Бот Закрыл + Бот Перевел")
    c3.metric("Бот (Закрыл)", count_bot_closed, f"{count_bot_closed/max(1, participated_count)*100:.1f}%")
    c4.metric("Люди (Всего)", count_human_chats)
    c5.metric("Авторизация", count_auth)
    ...
    with col_pies[0]:
        ...
        if total_chats_day > 0:
            # Данные для круга: 
            # 1. Бот закрыл 
            # 2. Бот перевел (но участвовал)
            # 3. Чистые люди (без участия бота)
            # 4. Авторизация
            labels = ['Бот (Закрыл)', 'Бот (Перевел)', 'Люди (Без бота)', 'Авторизация']
            sizes = [count_bot_closed, transferred_count, pure_human_chats, count_auth]
            colors = ['#ff9999', '#ffcc99', '#66b3ff', '#99ff99'] # Красный, Оранжевый, Синий, Зеленый
            ...
            wedges, texts, autotexts = ax1.pie(
                ...
                colors=colors, 
                startangle=90,
                pctdistance=0.85 # Смещаем проценты чуть к краю
            )
            
            # Добавляем "Дырку", чтобы получился Donut — так легче визуально выделить группы
            centre_circle = plt.Circle((0,0), 0.70, fc='white')
            fig1.gca().add_artist(centre_circle)
            
            # Выделяем "Участие бота" жирным в легенде или через аннотацию
            plt.tight_layout()
            st.pyplot(fig1, use_container_width=False)
            
            st.info(f"💡 Бот касался **{participated_count}** чатов из **{total_chats_day}**")
            
    with col_pies[1]:
        st.subheader("Эффективность бота")
        # Тут оставляем твой второй график, он хорош для детализации конверсии
        if participated_count > 0:
            st.caption(f"Анализ {participated_count} диалогов, где был бот")
            fig2, ax2 = plt.subplots(figsize=(4, 4))
            ax2.pie([count_bot_closed, transferred_count], 
                    labels=['Справился', 'Не справился'], 
