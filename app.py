...
def load_gsheet_data():
    try:
        ...
        # Твоя логика фикса пустых типов (Прямая маршрутизация)
        def fix_topic(row):
...
with tabs[2]:
    ...
    if not df_api.empty:
        ...
        if selected_dept:
            ...
            # --- KPI и Статистика (твой код остается без изменений) ---
            d_chats_api = dept_data['req_id'].nunique() 
            # ... (здесь расчет скорости и рейтингов) ...

            st.divider()
            
            # --- 5. ТЕМАТИКИ С ПРИОРИТЕТОМ СТАТУСА ---
            st.subheader("Тематика обращений (GSheet)")
            dept_gsheet = df_gsheet[df_gsheet['Отдел'] == selected_dept].copy()
            
            # Функция для формирования названия в стиле: "Категория (Тип обращения)"
            def get_smart_label(row):
                st_val = str(row.get('Статус', '-')).strip()
                st_lower = st_val.lower()
                tp_val = str(row.get('Тип обращения', '-')).strip()
                
                # Приоритет 1: Меню курьеров
                if 'меню курьер' in st_lower:
                    return f"Меню курьера ({tp_val})"
                
                # Приоритет 2: Ручники (Подтверждение)
                if 'ручник' in st_lower:
                    return f"Подтверждение заказов ({tp_val})"
                
                # Приоритет 3: Рассылки и макросы
                if 'рассылк' in st_lower or 'рассылк' in tp_val.lower():
                    return f"Ответ на рассылку ({tp_val})"
                if 'макрос' in st_lower or 'макрос' in tp_val.lower():
                    return f"Ответ на макрос ({tp_val})"
                
                # Приоритет 4: Если в статусе что-то есть, а в типе "Прямая маршрутизация"
                if st_val != '-' and 'Прямая маршрутизация' in tp_val:
                    return f"{st_val} ({tp_val})"

                return tp_val

            if not dept_gsheet.empty:
                # Генерируем финальные названия категорий
                dept_gsheet['Категория_Финальная'] = dept_gsheet.apply(get_smart_label, axis=1)
                
                cat_counts = dept_gsheet['Категория_Финальная'].value_counts().reset_index()
                cat_counts.columns = ['Категория', 'Кол-во']
                
                # Считаем Разницу: теперь она будет меньше, так как мы "опознали" курьеров
                ...
                st.warning(f"В таблице GSheet нет данных для отдела {selected_dept}. Вся нагрузка ({d_chats_api}) ушла в Разницу.")
