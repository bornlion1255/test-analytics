...
# TAB 3: DEPT ANALYSIS (Логика Тимлидов: Чернышова, Гетман, Власенкова)
with tabs[2]:
    st.subheader("Детальный анализ по отделу")
    if not df_api.empty:
        all_depts = sorted(df_api['Отдел'].unique())
        selected_dept = st.selectbox("Выберите отдел", all_depts, key="dept_analysis_v6")
        
        if selected_dept:
            dept_data = df_api[df_api['Отдел'] == selected_dept].copy()
            
            # Список Тимлидов (ищем по корню фамилии, чтобы не зависеть от е/о или регистра)
            ...
            if 'Дата' in dept_data.columns:
                # ЧАТЫ: Считаем всех (и спецов, и ТЛ)
                daily_chats = dept_data.groupby('Дата')['req_id'].nunique()
                # СПЕЦЫ: Считаем только тех, кто НЕ Тимлид
            ...
            for _, row in op_list.iterrows():
                s_avg = np.median(speeds_map.get(row['operator_id'], [])) if speeds_map.get(row['operator_id']) else None
                spec_rows.append({
...
# TAB 4: КАТЕГОРИИ (Интерактивный и полный)
with tabs[3]:
    ...
    if not df_gsheet.empty:
        ...
        with sub_tab1:
            ...
            st.dataframe(stats[['Всего', 'Бот(✓)', 'Бот(→)']].sort_values('Всего', ascending=False).reset_index(), use_container_width=True)

        # ГРАФИК (Plotly)
        with sub_tab2:
            top_names = df_gsheet['Тип обращения'].value_counts().nlargest(15).index
            df_plot = df_gsheet[df_gsheet['Тип обращения'].isin(top_names)].copy()
            df_plot['Результат'] = df_plot['Статус'].apply(lambda x: 'Бот справился' if x == 'Закрыл' else 'Перевод/Прочее')
            fig = px.bar(df_plot.groupby(['Тип обращения', 'Результат']).size().reset_index(name='Кол-во'), 
                         x="Кол-во", y="Тип обращения", color="Результат", orientation='h', text_auto=True,
                         category_orders={"Тип обращения": top_names.tolist()})
            fig.update_layout(height=700, barmode='stack', hovermode="y unified")
            st.plotly_chart(fig, use_container_width=True)
    else:
        st.info("Нет данных для анализа. Проверьте фильтр дат.")

# ==========================================
# TAB 5: ДИНАМИКА (НОВАЯ ЛОГИКА С 2 КАЛЕНДАРЯМИ)
# ==========================================
with tabs[4]:
    st.subheader("📈 Сравнение произвольных периодов")
    
    # 1. Инструкция по цветам (как на скриншоте)
    ...
    # 2. Выбор периодов через два календаря
    st.write("#### 1. Выберите периоды для анализа")
    col_date1, col_date2 = st.columns(2)
    
    today = datetime.now().date()
    
    with col_date1:
        st.markdown("**Текущий период (А)**")
        range_curr = st.date_input("Период А", [today - timedelta(days=7), today], key="range_a")
        
    with col_date2:
        st.markdown("**Прошлый период (Б)**")
        range_prev = st.date_input("Период Б", [today - timedelta(days=15), today - timedelta(days=8)], key="range_b")

    # Кнопка запуска (расчет начнется только здесь)
    run_analysis = st.button("🚀 Просчитать динамику и загрузить данные", use_container_width=True)
    ...
    if run_analysis:
        if len(range_curr) == 2 and len(range_prev) == 2:
            ...
            # Расчет данных (берем из общего массива df_gsheet_all)
            stats_curr = get_dynamics_stats(df_gsheet_all, c_s, c_e)
            stats_prev = get_dynamics_stats(df_gsheet_all, p_s, p_e)

            # Объединяем и СОРТИРУЕМ от большего к меньшему по объему
            ...
            def calculate_v_b(row):
                v_curr, v_prev = row['Всего_curr'], row['Всего_prev']
                b_curr, b_prev = row['Бот_%_curr'], row['Бот_%_prev']
                
                # V (Volume): Рост = 🔴, Снижение = 🟢
                v_pct = ((v_curr / v_prev - 1) * 100) if v_prev > 0 else (100.0 if v_curr > 0 else 0.0)
                v_ico = "🔴" if v_pct > 0 else "🟢"
                
                # B (Bot): Рост = 🟢, Снижение = 🔴
                b_pp = b_curr - b_prev
                b_ico = "🟢" if b_pp > 0 else ("🔴" if b_pp < 0 else "⚪")
            ...
            if not df_dyn.empty:
                res_table = df_dyn.apply(calculate_v_b, axis=1)
                res_table.columns = ['Период А (Текущий)', 'Период Б (Прошлый)', 'Динамика']
                
                # Добавляем строку ИТОГО
                t_v_curr = df_dyn['Всего_curr'].sum()
                t_v_prev = df_dyn['Всего_prev'].sum()
                
                def get_pct(d1, d2):
                    sub = df_gsheet_all[(df_gsheet_all['Дата'].dt.date >= d1) & (df_gsheet_all['Дата'].dt.date <= d2)]
                    return (sub['Статус'] == 'Закрыл').mean() * 100 if not sub.empty else 0

                t_b_curr, t_b_prev = get_pct(c_s, c_e), get_pct(p_s, p_e)
                t_v_pct = ((t_v_curr / t_v_prev - 1) * 100) if t_v_prev > 0 else 0
                
                total_row = pd.DataFrame([{
                    'Период А (Текущий)': f"{int(t_v_curr)}\n({t_b_curr:.1f}%)",
                    'Период Б (Прошлый)': f"{int(t_v_prev)}\n({t_b_prev:.1f}%)",
                    'Динамика': f"{'🔴' if t_v_pct > 0 else '🟢'} V: {t_v_pct:+.1f}%\n{'🟢' if (t_b_curr - t_b_prev) > 0 else '🔴'} B: {(t_b_curr - t_b_prev):+.1f}пп"
                }], index=['ИТОГО ПО ВСЕМ КАТЕГОРИЯМ'])
                
                res_table = pd.concat([res_table, total_row])
                
                st.success(f"Анализ завершен для периодов: {c_s}/{c_e} и {p_s}/{p_e}")
                st.dataframe(res_table, use_container_width=True, height=600)
            else:
                st.warning("Нет данных для анализа в эти даты.")
        else:
            st.error("Пожалуйста, выберите полные диапазоны дат в обоих календарях.")
# ==========================================
# TAB 6: БАЗА ДАННЫХ
# ==========================================
with tabs[5]: # 
