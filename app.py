...
# ==========================================
# TAB 3: DEPT ANALYSIS (ОБНОВЛЕННЫЙ)
# ==========================================
with tabs[2]:
    ...
    if not df_api.empty:
        ...
        selected_dept = st.selectbox("Выберите отдел", all_depts, key="dept_analysis_select_final")
        
        if selected_dept:
            # Фильтруем данные по отделу
            dept_data = df_api[df_api['Отдел'] == selected_dept].copy()
            
            # --- БЛОК 1: ОБЩАЯ СТАТИСТИКА (KPI ЗА ПЕРИОД) ---
            total_chats = dept_data['req_id'].nunique()
            unique_ops_total = dept_data['operator_id'].nunique()
            avg_load_total = total_chats / unique_ops_total if unique_ops_total > 0 else 0
            
            c1, c2, c3 = st.columns(3)
            c1.metric("Всего чатов за период", total_chats)
            c2.metric("Уникальных спецов за период", unique_ops_total)
            c3.metric("Ср. нагрузка на 1 чел.", f"{avg_load_total:.1f}")
            ...
            if 'Дата' in dept_data.columns:
                ...
                # Считаем нагрузку на специалиста в конкретный день
                daily_stats['Нагрузка на 1 чел.'] = (daily_stats['Чатов'] / daily_stats['Спецов']).round(1)
                
                # Выводим таблицу динамики
                st.dataframe(
                ...
                st.warning("В данных API отсутствует колонка 'Дата'. Проверьте функцию load_api_data_range.")

            st.divider()
            
            # --- БЛОК 3: ТАБЛИЦА СПЕЦИАЛИСТОВ (С ПОДСВЕТКОЙ ТИМЛИДОВ) ---
            ...
            for i, row in op_list.iterrows():
                ...
                # Логика определения роли и подсветки
            ...
            # Функция подсветки строк Тимлидов (нежно-голубой цвет)
        ...
        st.warning("Нет данных API. Нажмите 'Запустить анализ'.")
