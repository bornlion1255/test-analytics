...
# ==========================================
# 6. ВЫВОД ВКЛАДОК (ОТ 0 ДО 5)
# ==========================================

# СОЗДАЕМ ТАБЫ ОДИН РАЗ
...
with tabs[0]:
    ...
    with col_pies[1]:
        ...
        if not bot_participated_df.empty:
            part_count = len(bot_participated_df)
            trans_count = part_count - count_bot_closed
            fig2, ax2 = plt.subplots(figsize=(4, 4))
            ax2.pie([count_bot_closed, trans_count], labels=['Закрыл сам', 'Перевел'], 
...
with tabs[1]:
    ...
    if not df_api.empty:
        ...
        with c_heat:
            st.write("**Тепловая карта: Отдел vs Час**")
            hm_df = df_api[df_api['Час'].between(0, 23)]
            if not hm_df.empty:
                hm_data = hm_df.groupby(['Отдел', 'Час'])['req_id'].nunique().unstack(fill_value=0).reindex(columns=range(24), fill_value=0)
...
# TAB 3: DEPT ANALYSIS (Логика Тимлидов: Чернышева, Гетман, Власенкова)
with tabs[2]:
    st.subheader("Детальный анализ по отделу")
    if not df_api.empty:
        all_depts = sorted(df_api['Отдел'].unique())
        selected_dept = st.selectbox("Выберите отдел", all_depts, key="dept_analysis_final_idx")
        
        if selected_dept:
            dept_data = df_api[df_api['Отдел'] == selected_dept].copy()
            TL_LIST = ["Чернышева", "Гетман", "Власенкова"]
            dept_data['is_tl'] = dept_data['Оператор'].apply(lambda x: any(tl.lower() in str(x).lower() for tl in TL_LIST))

            if 'Дата' in dept_data.columns:
                daily_chats = dept_data.groupby('Дата')['req_id'].nunique()
                # Спецы — только те, кто НЕ Тимлид
...
with tabs[3]:
    ...
    if not df_gsheet.empty:
        with sub_tab1:
            cat_stats = df_gsheet.groupby(['Тип обращения', 'Статус']).size().unstack(fill_value=0)
            cat_stats['Всего'] = cat_stats.sum(axis=1)
            for c in ['Закрыл', 'Перевод']: 
                if c not in cat_stats.columns: cat_stats[c] = 0
            cat_stats['Бот(✓)'] = (cat_stats['Закрыл'] / cat_stats['Всего'] * 100).map('{:.1f}%'.format)
            cat_stats['Бот(→)'] = (cat_stats['Перевод'] / cat_stats['Всего'] * 100).map('{:.1f}%'.format)
            st.dataframe(cat_stats[['Всего', 'Бот(✓)', 'Бот(→)']].sort_values('Всего', ascending=False).reset_index(), use_container_width=True, hide_index=True)
        with sub_tab2:
            ...
            fig = px.bar(df_plot.groupby(['Тип обращения', 'Результат']).size().reset_index(name='Кол-во'), 
                         x="Кол-во", y="Тип обращения", color="Результат", orientation='h', text_auto=True, category_orders={"Тип обращения": top_names.tolist()})
            fig.update_layout(height=700, barmode='stack')
            st.plotly_chart(fig, use_container_width=True)

# TAB 5: ДИНАМИКА (V 🔴 рост / B 🟢 рост)
with tabs[4]:
    ...
    mode = st.selectbox("Режим сравнения:", ["День к дню", "Неделя к неделе", "Месяц к месяцу", "Свой период"], index=1, key="dyn_mode")
    today_dyn = datetime.now().date()
    
    # Логика расчета дат
    if mode == "День к дню":
        c_s, c_e = today_dyn - timedelta(days=1), today_dyn - timedelta(days=1)
        p_s, p_e = today_dyn - timedelta(days=2), today_dyn - timedelta(days=2)
    elif mode == "Неделя к неделе":
        c_s = today_dyn - timedelta(days=today_dyn.weekday() + 7); c_e = c_s + timedelta(days=6)
        p_s = c_s - timedelta(days=7); p_e = p_s + timedelta(days=6)
    elif mode == "Месяц к месяцу":
        c_e = today_dyn.replace(day=1) - timedelta(days=1); c_s = c_e.replace(day=1)
        p_e = c_s - timedelta(days=1); p_s = p_e.replace(day=1)
    else:
        c_s, c_e = sel_start, sel_end # Используем те, что в сайдбаре
        p_s, p_e = c_s - (c_e - c_s) - timedelta(days=1), c_s - timedelta(days=1)

    st.info(f"Сравнение: **{c_s} / {c_e}** vs **{p_s} / {p_e}**")
    if st.button("🚀 Запустить динамический анализ"):
        s_curr = get_dynamics_stats(df_gsheet_all, c_s, c_e)
        s_prev = get_dynamics_stats(df_gsheet_all, p_s, p_e)
        df_dyn = s_curr.join(s_prev, lsuffix='_curr', rsuffix='_prev', how='outer').fillna(0).sort_values('Всего_curr', ascending=False)
        
        def calc_dyn(row):
            v_p = ((row['Всего_curr'] / row['Всего_prev'] - 1) * 100) if row['Всего_prev'] > 0 else 0
            b_p = row['Бот_%_curr'] - row['Бот_%_prev']
            return pd.Series([f"{int(row['Всего_curr'])}\n({row['Бот_%_curr']:.1f}%)", f"{int(row['Всего_prev'])}\n({row['Бот_%_prev']:.1f}%)", 
                              f"{'🔴' if v_p > 0 else '🟢'} V: {v_p:+.1f}%\n{'🟢' if b_p > 0 else '🔴'} B: {b_p:+.1f}пп"])
        
        res = df_dyn.apply(calc_dyn, axis=1)
...
with tabs[5]:
    ...
    if not df_gsheet.empty:
        ...
        st.info("Данные не загружены. Нажмите 'Запустить анализ' в меню слева.")