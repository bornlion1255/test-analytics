...
with tabs[2]:
    ...
    if not df_api.empty:
        ...
        selected_dept = st.selectbox("Выберите отдел", all_depts, key="dept_analysis_v10")
        ...
        if selected_dept:
            ...
            # --- ГЕНЕРАЦИЯ МИКРО-ОТЧЕТА С ДИНАМИКОЙ ---
            def calc_metrics_for_report(df, sm, fsm, dept_name):
                if df.empty: return None
                chats = df['req_id'].nunique()
                ratings = pd.to_numeric(df.drop_duplicates('req_id')['rating'], errors='coerce').dropna()
                ...
                daily_c = df.groupby('Дата')['req_id'].nunique()
                if dept_name == "Бот AI":
                    daily_o = pd.Series(1, index=daily_c.index)
                else:
                    daily_o = df[~df['is_tl']].groupby('Дата')['operator_id'].nunique()
...
Средняя скорость: {format_seconds(curr_m['speed'])}{fmt_trend(curr_m['speed'], prev_m['speed'] if prev_m else 0, is_time=True)}"""
            ...
            # --- ТЕМАТИКИ С РАСЧЕТОМ РАЗНИЦЫ (API vs SHEET) ---
            st.subheader("Тематика обращений (GSheet)")
            dept_gsheet = df_gsheet[df_gsheet['Отдел'] == selected_dept].copy()
            
            # Универсальное правило обработки статусов (Курьеры, Ручники и тд)
            ...
            if not dept_gsheet.empty:
                ...
                # Считаем разницу, отнимая от API всё, что удалось вытащить из таблицы
