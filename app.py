...
with tabs[0]:
    st.subheader("📊 Единое распределение нагрузки")

    # --- 1. СБОР ДАННЫХ ПО ГРУППАМ ---
    
    # ГРУППА А: ЧЕЛОВЕК (Синие тона)
    human_data = {}
    
    # 1.1. Отделы из API (Люди)
    if not df_api.empty:
        human_data.update(df_api.groupby('Отдел')['req_id'].nunique().to_dict())

    # 1.2. Категории "Без AI" из GSheet (Человек)
    no_ai_mask = df_gsheet['Статус'].str.contains('Без AI', case=False, na=False)
    no_ai_counts = df_gsheet[no_ai_mask]['Тип обращения'].value_counts().to_dict()
    for cat, count in no_ai_counts.items():
        human_data[f"📝 {cat} (Человек)"] = count

    # 1.3. Ошибка авторизации (Человек)
    auth_fail_mask = df_gsheet['Тип обращения'].str.contains('Авторизация не пройдена', case=False, na=False)
    count_auth_failed = len(df_gsheet[auth_fail_mask])
    if count_auth_failed > 0:
        human_data["⚠️ Авторизация не пройдена (Человек)"] = count_auth_failed

    # ГРУППА Б: БОТ (Красные/Розовые тона)
    bot_data = {}

    # 2.1. Бот принимал участие: Закрыл сам
    count_bot_closed = len(df_gsheet[df_gsheet['Статус'].str.lower() == 'закрыл'])
    if count_bot_closed > 0:
        bot_data["🤖 Бот: Закрыл сам"] = count_bot_closed

    # 2.2. Бот принимал участие: Перевел на оператора (входит в общее участие бота)
    count_bot_transfer = len(df_gsheet[df_gsheet['Статус'].str.lower() == 'перевод'])
    if count_bot_transfer > 0:
        bot_data["🤖 Бот: Участие + Перевод"] = count_bot_transfer

    # 2.3. Авторизация пройдена (Бот)
    auth_pass_mask = df_gsheet['Тип обращения'].str.contains('Авторизация пройдена', case=False, na=False)
    count_auth_passed = len(df_gsheet[auth_pass_mask])
    if count_auth_passed > 0:
        bot_data["🤖 Бот: Авторизация пройдена"] = count_auth_passed

    # ГРУППА В: СИСТЕМА (Серый)
    system_data = {}
    stub_mask = df_gsheet['Тип обращения'].str.contains('Заглушка', case=False, na=False)
    count_stubs = len(df_gsheet[stub_mask])
    if count_stubs > 0:
        system_data["⚙️ Система: Заглушка"] = count_stubs

    # --- 2. ПОДГОТОВКА ЦВЕТОВ И МЕТОК ---
    labels = list(human_data.keys()) + list(bot_data.keys()) + list(system_data.keys())
    values = list(human_data.values()) + list(bot_data.values()) + list(system_data.values())
    
    # Формируем палитру: Синие для людей -> Красные для бота -> Серый для системы
    colors = (
        ['#1f77b4', '#3498db', '#5dade2', '#85c1e9', '#aed6f1', '#2e86c1'] * 5
    )[:len(human_data)] + (
        ['#e74c3c', '#f1948a', '#ff9999'] # Красный (Закрыл), Светло-красный (Перевод), Розовый (Авто)
    )[:len(bot_data)] + ['#bdc3c7'] # Серый

    # --- 3. ВИЗУАЛИЗАЦИЯ (Сплошной круг) ---
    if sum(values) > 0:
        fig = go.Figure(data=[go.Pie(
            labels=labels, 
            values=values, 
            hole=0, # Сплошной круг
            marker_colors=colors,
            textinfo='none', # Убираем текст внутри круга
            hoverinfo='label+percent+value',
            sort=False # Сохраняем порядок групп
        )])

        fig.update_layout(
            height=800, # Увеличенный размер
            ...
            title=dict(
                text="<b>Общее распределение нагрузки: Человек vs Бот vs Система</b>",
                x=0.5,
                font=dict(size=22)
            )
        )
    ...
    total_chats = sum(values)
    human_total = sum(human_data.values())
    bot_total = sum(bot_data.values())
    
    c1.metric("Всего чатов", total_chats)
    c2.metric("Участие людей", human_total, help="Отделы API + категории Без AI + ошибки авторизации")
    c3.metric("Участие бота", bot_total, f"{round(bot_total/total_chats*100, 1)}% от всех")
    c4.metric("Без участия (Система)", count_stubs)
...
# ==========================================
# TAB 3: DEPT ANALYSIS (TL + EVALS + GAP API/SHEET)
# ==========================================
with tabs[2]:
    st.subheader("Детальный анализ по отделу")
    if not df_api.empty:
        all_depts = sorted(df_api['Отдел'].unique())
        selected_dept = st.selectbox("Выберите отдел", all_depts, key="dept_analysis_v8")
        ...
        if selected_dept:
            ...
            **(По отделу: Ср. скорость: {format_seconds(d_med)} | Рейтинг: {d_rate_str} ({d_rate_cnt} шт.))**
            ...
            # --- 5. ТЕМАТИКИ С РАСЧЕТОМ РАЗНИЦЫ (API vs SHEET) ---
            ...
            unknown_gap = max(0, d_chats_api - total_sheet)
            ...
            cat_counts['Доля'] = (cat_counts['Кол-во'] / d_chats_api * 100).map('{:.1f}%'.format)
