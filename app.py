        ...
        if selected_dept:
            ...
            **(По отделу: Ср. скорость: {format_seconds(d_med)} | Рейтинг: {d_rate_str} ({d_rate_cnt} шт.))**
            ...
            if not dept_gsheet.empty:
                # Внедряем "Умные категории" на лету для детализации
                def map_smart_category(row):
                    status = str(row.get('Статус', ''))
                    topic = str(row.get('Тип обращения', ''))
                    
                    if status == 'Меню курьеров': return '🚚 Меню курьеров'
                    if status in ['Ручник: Позовите человека', 'Ручник: Обзвон и отмены']: return '✅ Подтверждение заказов'
                    if 'Ответ на рассылку' in topic or 'Ответ на рассылку' in status: return '📩 Ответ на рассылку'
                    if 'Ответ на макрос' in topic or 'Ответ на макрос' in status: return '⌨️ Ответ на макрос'
                    return topic

                dept_gsheet['Отработанная_категория'] = dept_gsheet.apply(map_smart_category, axis=1)
                
                cat_counts = dept_gsheet['Отработанная_категория'].value_counts().reset_index()
                cat_counts.columns = ['Категория', 'Кол-во']
                
                total_sheet = len(dept_gsheet)
                # Разница считается только ПОСЛЕ учета всех строк из GSheet
                ...
                # Если в GSheet по этому отделу пусто, вся нагрузка API идет в разницу
                st.write(f"Нет данных в GSheet для отдела {selected_dept}. Разница (API): {d_chats_api}")
